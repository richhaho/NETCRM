...
class Departments extends Admin_Controller
{
    ...
    public function save_departments($id = NULL)
    {
                ...
                foreach ($menu['menu'] as $key => $v_menu) {
                    if ($v_menu != 'undefined') {
                        if ($v_menu != 1 || $v_menu != 2) {
                            ...
                            $mdata['view'] = ($view ? $view : 0);
                            $mdata['created'] = ($created ? $created : 0);
                            $mdata['edited'] = ($edited ? $edited : 0);
                            $mdata['deleted'] = ($deleted ? $deleted : 0);
        ...
        redirect($_SERVER['HTTP_REFERER']); //redirect page
    }
