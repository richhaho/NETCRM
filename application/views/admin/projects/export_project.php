...
<br/>
<table class="items" width="100%" page-break-inside: auto;>
    <thead>
    <tr>
        <th><?= lang('name') ?></th>
            ...
            foreach ($total_timer as $v_timer) {
                ...
                if (!empty($aproject_info)) {
                    ...
                    <tr>
                        ...
                        <td style="text-align: center"><?= strftime(config_item('date_format'), strtotime($v_timer->start_time)) . ' ' . display_time($v_timer->start_time, true) ?></td>
                        <td style="text-align: center"><?= strftime(config_item('date_format'), strtotime($v_timer->end_time)) . ' ' . display_time($v_timer->end_time, true) ?></td>
                    ...
                    foreach ($total_tasks_timer as $v_task_timer) {
                        ...
                        if (!empty($task_info)) {
                            ...
                            <tr>
                                <td><?= $profile_info->fullname ?></td>
                                <td><?= $v_u_tasks->task_name ?></td>
                                <td style="text-align: center"><?= strftime(config_item('date_format'), strtotime($v_task_timer->start_time)) . ' ' . display_time($v_task_timer->start_time, true) ?></td>
                                <td style="text-align: center"><?= strftime(config_item('date_format'), strtotime($v_task_timer->end_time)) . ' ' . display_time($v_task_timer->end_time, true) ?></td>
