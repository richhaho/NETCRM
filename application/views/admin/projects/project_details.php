...
if (strtotime(strftime(config_item('date_format'))) > strtotime($project_details->end_date . '00:00')) {
    $lang = lang('days_gone');
} else {
    $lang = lang('days_left');
}
                    ...
                    <div class="tab-content bg-white">
                                            ...
                                            if (!empty($aproject_info)) {
                                                if ($v_timer->start_time != 0 && $v_timer->end_time != 0) {
                                                    ...
                                                    <tr id="table-timesheet-<?= $v_timer->tasks_timer_id ?>">
                                                        ...
                                                        <td><span
                                                                class="label label-success"><?= strftime(config_item('date_format'), $v_timer->start_time) . ' ' . display_time($v_timer->start_time, true) ?></span>
                                                        </td>
                                                        <td><span
                                                                class="label label-danger"><?= strftime(config_item('date_format'), $v_timer->end_time) . ' ' . display_time($v_timer->end_time, true) ?></span>
                                                        </td>
                        ...
                        <div class="tab-pane <?= $time_active == 2 ? 'active' : ''; ?>" id="contact">
                            <form role="form" enctype="multipart/form-data" id="form"
                                ...
                                <?php
                                if (!empty($project_timer_info)) {
                                    $start_date = strftime(config_item('date_format'), $project_timer_info->start_time);
                                    $start_time = date(strtotime(strftime(config_item('time_format'))), $project_timer_info->start_time);
                                    $end_date = strftime(config_item('date_format'), $project_timer_info->end_time);
                                    $end_time = date(strtotime(strftime(config_item('time_format'))), $project_timer_info->end_time);
                                } else {
                                    $start_date = '';
                                    $start_time = '';
                                    $end_date = '';
