...
<div class="row">
    ...
    if (!empty($all_job_circular)):foreach ($all_job_circular as $v_job_circular):
        ...
        $current_time = strftime(config_item('date_format'));
        if ($current_time > $last_date) {
            ...
            $lastdate = strftime(config_item('date_format'), strtotime($v_job_circular->last_date));
            $today = strftime(config_item('date_format'));
