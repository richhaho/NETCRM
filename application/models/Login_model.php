...
class Login_Model extends MY_Model
{
    ...
    function set_password_key($user_id, $new_pass_key)
    {
        ...
        $this->db->set('new_password_requested', strftime(config_item('date_format', 'time_format')));
        $this->db->where('user_id', $user_id);
        $this->db->update('tbl_users');
            ...
            foreach ($all_clock_out as $clock_out) {
                if (!empty($clock_out)) {
                    ...
                    if (empty($attendance_info->date_in)) {
                        $adata['date_in'] = strftime(config_item('date_format'));
                    }
                    $adata['clocking_status'] = 0;
                    $adata['date_out'] = strftime(config_item('date_format'));
