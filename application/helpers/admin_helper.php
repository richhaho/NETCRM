...
function display_time($value, $no_str = null)
{
    ...
    if (!empty($check_day1[1]) || !empty($check_day2[1])) {
        $time = time();
    }
    ...
    if (!empty($all_field)) {
        foreach ($all_field as $v_fileds) {
            if (!empty($v_fileds->visible_for_admin)) {
                ...
                <input type="text" name="' . $name . '" class="form-control datepicker" value="' . (!empty($value) ? $value : strftime(config_item('date_format'))) . '">
...
function add_notification($values)
{
    ...
    $data['date'] = strftime(config_item('date_format', 'time_format'));
...
function daysleft($time)
{
    ...
    $cur_date = strtotime(strftime(config_item('date_format')));
